<?php
abstract class Shahid_Social_Service_Abstract
{
    ...
    public function setTokenHandler($param)
    {
        ...
        if ($param instanceof Shahid_Social_TokenHandler_Abstract) {
            $this->_tokenHandler = $param;
        } elseif (is_array($param)) {
            $this->_tokenHandler = Shahid_Social_TokenHandler_Factory::getHandler($param);
        }
    ...
    {
        if (null === $this->_tokenHandler) {
            $params = array(Shahid_Social_TokenHandler_Factory::SESSION, get_class($this));
...
//                if ($throwException)
                throw new Shahid_Social_Exception(sprintf("map contains invalid key (%s)", $key));
            }
        }
    }
