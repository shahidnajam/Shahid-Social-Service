<?php
class Shahid_Social_Service_Facebook extends Shahid_Social_Service_Abstract
{
    ...
    public function publish($message)
    {
        foreach($message as $key => $params) {
            if (null !== $this->_mapReference[$key] && (strlen($params) > $this->_mapReference[$key]))
                $message[$key] = Shahid_Inflector::cuttext($params, $this->_mapReference[$key]);
        }

        if (!$this->getTokenHandler()->getToken())
            throw new Shahid_Social_NoTokenException('No access token');
        
        $target = $this->getTargetObject();
        if (null === $target || '' == trim($target))
            throw new Shahid_Social_Exception('No target for publishing has been defined');
        ...
        {            
            if ($data['error']['type'] == 'Exception') {
                ...
                if (strpos($data['error']['message'],'#200')) {
                    throw new Shahid_Social_NoTokenException($data['error']['message']);
                }
                if (strpos($data['error']['message'],'#210')) {
                    throw new Shahid_Social_NoTokenException($data['error']['message']);
                }
                throw new Shahid_Social_Exception($data['error']['message']);
...
//                }
    ...
    public function setOptions($options)
    {
        ...
        if (isset($options['requested_permissions'])) {
            ...
            throw new Shahid_Social_Exception('Invalid parameters : param \'requested_permissions\' is missing');
        }
        ...
        if (isset($options['client_id'])) {
            ...
            throw new Shahid_Social_Exception('Invalid parameters : param \'client_id\' is missing');
        }
        ...
        if (isset($options['client_secret'])) {
            ...
            throw new Shahid_Social_Exception('Invalid parameters : param \'client_secret\' is missing');
        }
        ...
        if (isset($options['callback_uri'])) {
            ...
            throw new Shahid_Social_Exception('Invalid parameters : param \'callback_uri\' is missing');
        }
