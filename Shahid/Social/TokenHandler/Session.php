<?php
class Shahid_Social_TokenHandler_Session extends Shahid_Social_TokenHandler_Abstract
{
    
    const DEFAULT_NAMESPACE = 'Shahid_Social_TokenHandler';
    ...
    public function setOptions($params)
    {
        ...
        if (is_array($params)) {
            if (!isset($params[self::PARAM_SERVICE]) || '' === trim((string) $params[self::PARAM_SERVICE]))
                throw new Shahid_Social_TokenHandler_Exception('Invalid params given to the Session token Handler');
